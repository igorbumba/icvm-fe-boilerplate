...
_b.AutoSuggest.prototype.doAjaxRequest = function(input) {
    ...
    if (Object.prototype.toString.call(url) === '[object Array]') {
        pointer.setSuggestions(url, input);
    } else {
        if (!url) return false;
    ...
    if (Object.prototype.toString.call(req) === '[object Array]') {
        ...
        for (var i = 0; i < jsondata.results.length; i++) {
            this.aSug.push({
                ...
                resultspage: jsondata.results[i].resultspage,
            });
        }
    } else {
        ...
        for (var i = 0; i < results.length; i++) {
            if (results[i].hasChildNodes())
                this.aSug.push({
                    ...
                    resultspage: results[i].getAttribute('resultspage'),
                });
        }
    }
...
_b.AutoSuggest.prototype.createList = function(arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        var span = _b.DOM.cE('span', { className: 'suglst' }, output, true);
        if (arr[i].id != '') {
            if (arr[i].info != '') {
                var br = _b.DOM.cE('br', {});
                span.appendChild(br);
            ...
            var a = _b.DOM.cE('a', { href: 'javascript:void(0)', id: arr[i].value });
            ...
            a.name = i + 1;
            a.onclick = function() {
                document.getElementById('searchkey').value = this.getAttribute('id');
                document.getElementById('as_searchkey').remove();
...
function hiliter(word) {
    ...
    $('.suglst').each(function(i, el) {
        var rgxp = new RegExp(kword, 'gi');
        var repl = '<em>' + kword + '</em>';

        if (rgxp.test($(el).text())) {
            var nword = $(el)
                .text()
                .replace(rgxp, repl);
            $(el).html(nword);
